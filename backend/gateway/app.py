...
async def search_stocks(keyword: str = Query(..., min_length=1, description="搜索关键词（股票代码或名称）")):
    ...
    try:
        ...
        # 判断是否为纯数字（股票代码搜索）
        is_code_search = keyword.isdigit()
        # 判断是否为纯中文（不包含数字和英文）
        is_chinese_search = all('\u4e00' <= char <= '\u9fff' for char in keyword) and not any(c.isdigit() or c.isalpha() for c in keyword)
    ...
    try:
        ...
        for stock in selected:
            ...
            if indicators:
                ...
                stock["bias"] = indicators.get("bias")
                stock["williams_r"] = indicators.get("williams_r")
                stock["boll_upper"] = indicators.get("boll_upper")
                stock["boll_middle"] = indicators.get("boll_middle")
                stock["boll_lower"] = indicators.get("boll_lower")
                stock["ema12"] = indicators.get("ema12")
                stock["ema26"] = indicators.get("ema26")
...
async def get_db_info_api():
    ...
    try:
        ...
        # 1. 检测价格<=0的数据
        price_zero = client.execute("""
            SELECT count() FROM kline FINAL WHERE close <= 0 OR open <= 0
        """)
        if price_zero and price_zero[0][0] > 0:
            anomalies.append({
                "type": "price_zero",
                "count": price_zero[0][0],
...
async def collect_spot_data_api(
    ...
    try:
        ...
        from market.service.sse import broadcast_message
        from market.service.ws import spot_collect_progress, spot_collect_stop_flags
        from common.runtime_config import get_runtime_config
        import uuid
        from datetime import datetime
        ...
        def collect_batch_internal():
                ...
                try:
                    for i in range(0, len(a_codes), batch_size):
                        ...
                        for future in concurrent.futures.as_completed(futures):
                            ...
                            current_source = last_used_source_a[0] or ""
                            kline_collect_progress[task_id].update({
                                ...
                                "progress": progress_pct,
                                "data_source": current_source,
                                "message": f"A股采集中({current_source})... 已处理{total_processed}/{total_stocks}，成功{total_success}，失败{total_failed}"
                            })
                    ...
                    for i in range(0, len(hk_codes), batch_size):
                        ...
                        for future in concurrent.futures.as_completed(futures):
                            ...
                            kline_collect_progress[task_id].update({
                                ...
                                "progress": progress_pct,
                                "data_source": current_source,
                                "message": f"港股采集中({current_source})... 已处理{total_processed}/{total_stocks}，成功{total_success}，失败{total_failed}"
                            })
            ...
            if is_cancelled:
                ...
                kline_collect_progress[task_id] = {
                    ...
                    "failed": total_failed,
                    "current": total_processed,
                    "message": f"采集已停止({final_source})！成功{total_success}，失败{total_failed}，已处理{total_processed}/{total_stocks}",
                    "start_time": start_time,
                    "end_time": end_time,
                ...
                kline_collect_progress[task_id] = {
                    ...
                    "failed": total_failed,
                    "current": total_processed,
                    "message": f"采集完成({final_source})！成功{total_success}，失败{total_failed}，总计{total_processed}",
                    "start_time": start_time,
                    "end_time": end_time,
                    "progress": 100,
                    "data_source": final_source
...
async def stop_kline_collect_api():
    ...
    try:
        ...
        running_spot_tasks = [
            (task_id, progress) 
            for task_id, progress in spot_collect_progress.items()
            if progress.get("status") == "running"
        ]
        
        # 停止K线采集任务
        for task_id, progress in running_kline_tasks:
            ...
            stopped_tasks.append(f"K线采集:{task_id}")
            logger.info(f"用户停止K线采集任务: {task_id}")
        
        # 停止实时行情采集任务
        for task_id, progress in running_spot_tasks:
            spot_collect_stop_flags[task_id] = True
            spot_collect_progress[task_id].update({
                "status": "cancelled",
                "message": "用户手动停止采集任务",
                "end_time": datetime.now().isoformat()
            })
        ...
        return {
            ...
            "message": f"已发送停止信号，{len(stopped_tasks)}个任务将停止"
        }
