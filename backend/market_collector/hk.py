...
def fetch_hk_stock_spot() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # 存储到Redis
        set_json("market:hk:spot", result, ex=300)
        get_redis().set("market:hk:time", datetime.now().isoformat(), ex=300)
        
        logger.info(f"港股行情采集成功，共{len(result)}只股票")
        return result
        
    except Exception as e:
        logger.error(f"港股行情采集失败: {e}", exc_info=True)
        return []
